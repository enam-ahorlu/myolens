...
|---|---|---|---|---|---|---|---|
| ~~TD-01~~ | ~~No held-out validation set~~ | n/a | n/a | n/a | ~~Critical~~ | n/a | **Resolved before the window.** Deployment models retrained on 37 of 40 subjects; subjects 10, 13 and 22 held out and used to verify deployed accuracy independently. |
...
| ~~TD-05~~ | ~~ONNX↔native equivalence unproven~~ | n/a | n/a | Reckless & Inadvertent | ~~Critical~~ | ~~Critical~~ | **Paid.** Max absolute delta 4.346e-07 over 200 windows, both models, 100% argmax agreement, in the serving runtime. Tolerance 1e-4 was not relaxed. Regression test ships in `backend/tests/`. |
| TD-06 | Demonstration fixtures double as test fixtures, **and now as the measurement set for a reported result** | Expedient, since the held-out subjects were the cheapest realistic data available | Tests may encode fixture quirks rather than requirements, and D6's published bout-coherence figures inherit any such quirk | Prudent & Inadvertent | **Scheduled** | ~~Low~~ **Medium** | v1.1, synthetic edge-case fixtures for the boundary conditions, and an independent set for any figure that is reported rather than merely asserted |
